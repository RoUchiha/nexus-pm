...
export function ConnectionsPanel({ connectors, actions, locked }: Props) {
  ...
  const readyCount = connectors.filter(
    (item) => item.enabled && item.approved && item.status === 'ready',
  ).length;
      ...
      >
        <span>
          ...
          <span className="connections-sub">
            {readyCount}/{connectors.length} approved and routable · secrets memory-only
          </span>
        </span>
      ...
      {expanded && (
        <div className="connections-body">
          <div className="connector-agent-banner">
            ...
            <span>
              Validates endpoints, scopes, credentials, approvals, and routes. Production execution
              goes through a server-side broker—never directly from this browser.
            </span>
          </div>
          ...
          {!locked && (
            <div className="connector-add-row">
              ...
              <button className="btn btn-primary" onClick={() => actions.add(definitionId)}>
                Add connection
              </button>
            </div>
          )}
          ...
          <div className="connector-list">
            {connectors.length === 0 && (
              <div className="worker-empty">No external connections configured.</div>
            )}
              ...
              return (
                <article className={`connector-card ${connector.status}`} key={connector.id}>
                  <div className="connector-card-header">
                    <div>
                      ...
                      <div className="connector-meta">
                        {definition?.kind ?? 'unknown'} · {definition?.description}
                      </div>
                    </div>
                  ...
                  <div className="connector-grid">
                    ...
                    <label>
                      ...
                      <select
                        ...
                        disabled={locked}
                        onChange={(event) =>
                          updateAuth(connector, event.target.value as ConnectorAuthType)
                        }
                      >
                        {(definition?.authTypes ?? ['none']).map((auth) => (
                            ...
                            onChange={(event) =>
                              actions.update(connector.id, {
                                credentials: {
                                  ...
                                  [secretField]: event.target.value.slice(0, 4096),
                                },
                                approved: false,
                                status: 'draft',
                              })
                    ...
                    {connector.authType === 'basic' && (
                      <>
                        <label>
                          ...
                          <input
                            className="input"
                            value={connector.credentials.username ?? ''}
                            onChange={(event) =>
                              actions.update(connector.id, {
                                credentials: {
                                  ...connector.credentials,
                                  username: event.target.value.slice(0, 256),
                                },
                                approved: false,
                                status: 'draft',
                              })
                        ...
                        <label>
                          ...
                          <input
                            ...
                            type="password"
                            autoComplete="off"
                            value={connector.credentials.password ?? ''}
                            onChange={(event) =>
                              actions.update(connector.id, {
                                credentials: {
                                  ...connector.credentials,
                                  password: event.target.value.slice(0, 4096),
                                },
                                approved: false,
                                status: 'draft',
                              })
                    ...
                    <label>
                      ...
                      <select
                        className="input"
                        value={connector.controlMode}
                        onChange={(event) =>
                          actions.setControlMode(
                            connector.id,
                            event.target.value as ConnectorControlMode,
                          )
                        }
                      >
                  ...
                  <label className="connector-steering">
                    ...
                    <textarea
                      className="input"
                      placeholder="Constraints, routing preference, or debugging context"
                      value={connector.steeringNotes}
                      onChange={(event) => actions.steer(connector.id, event.target.value)}
                    />
                  </label>

                  {connector.diagnostics.length > 0 && (
                    <div className="connector-diagnostics">
                      {connector.diagnostics.map((item, index) => (
                  ...
                  {connector.issues.map((issue) => (
                    <details
                      className={`connector-issue ${issue.severity}`}
                    ...
                    >
                      ...
                      <p>{issue.detail}</p>
                      <ol>
                        {issue.remediation.map((step, index) => (
                  ...
                  <div className="connector-actions">
                    <button
                      ...
                      disabled={locked || connector.status === 'checking'}
                      onClick={() => actions.diagnose(connector.id)}
                    >
                      Diagnose
                    </button>
                    <button
                      ...
                      disabled={
                        locked ||
                        connector.approved ||
                        connector.issues.some((issue) => issue.severity === 'error')
                      }
                      onClick={() => actions.approve(connector.id)}
                    >
                      Approve
                    </button>
                    {connector.status === 'paused' ? (
                      <button
                        className="btn btn-ghost"
                        onClick={() => actions.resume(connector.id)}
                      >
                        Resume
                      </button>
                    ) : (
                      <button className="btn btn-ghost" onClick={() => actions.pause(connector.id)}>
                        Pause
                      </button>
                    )}
                    <button
                      className="btn btn-danger"
                      onClick={() => actions.takeOver(connector.id)}
                    >
                      Take over
                    </button>
                    {!locked && (
                      <button
                        className="btn btn-ghost connector-remove"
                        onClick={() => actions.remove(connector.id)}
                      >
                        Remove
                      </button>
                    )}
          ...
          <div className="connector-router">
            ...
            <div className="routing-results">
              {routePlan.routes.map((route) => (
                <span key={route.capability}>
                  {route.capability} → {route.connectorName}
                  {route.requiresApproval ? ' · approval' : ''}
                </span>
              ))}
              {routePlan.unresolvedCapabilities.map((capability) => (
                <span className="unresolved" key={capability}>
