...
export async function runDemoReplay(
  ...
  dispatch({
    ...
    entry: makeActivityEntry(
      'nexus-manager', 'NEXUS Manager', 'spec_drafting', 'spec_drafted',
  ...
  const pods = DEMO_DISCOVERY.pods.map(bp => ({
    ...bp, status: 'queued' as const, output: '', logs: [], retries: 0,
  }));
  ...
  for (const podId of wave1) {
    dispatch({ type: 'UPDATE_POD', id: podId, patch: { status: 'waiting', startTime: Date.now() } });
  }
  await sleep(200);
  ...
  await Promise.all(wave1.map(async podId => {
    await streamText(
    ...
    if (signal.aborted) return;
    dispatch({ type: 'UPDATE_POD', id: podId, patch: { status: 'completed', endTime: Date.now(), usedProvider: 'anthropic' } });
    dispatch({
      type: 'ADD_ACTIVITY_LOG',
      entry: makeActivityEntry(
        `pod:${podId}`,
        DEMO_DISCOVERY.pods.find(p => p.id === podId)!.name,
        'executing', 'pod_completed',
        DEMO_DISCOVERY.pods.find(p => p.id === podId)!.responsibility,
  ...
  const directives = DEMO_WAVE_CHECK_DIRECTIVES.map(d => ({
    ...d, id: nextBusId(), timestamp: Date.now(),
  }));
  addBus(directives);
  ...
  dispatch({
    type: 'ADD_ACTIVITY_LOG',
    entry: makeActivityEntry(
      'nexus-manager', 'NEXUS Manager',
      'executing', 'manager_directive',
  ...
  dispatch({ type: 'UPDATE_POD', id: 'frontend_ui', patch: { status: 'waiting', startTime: Date.now() } });
  await sleep(200);
  dispatch({ type: 'UPDATE_POD', id: 'frontend_ui', patch: { status: 'running' } });
  dispatch({
    type: 'ADD_ACTIVITY_LOG',
    entry: makeActivityEntry(
      'pod:frontend_ui', 'Frontend UI Pod',
      'executing', 'pod_started',
  ...
  dispatch({ type: 'UPDATE_POD', id: 'frontend_ui', patch: { status: 'completed', endTime: Date.now(), usedProvider: 'anthropic' } });
  dispatch({
    type: 'ADD_ACTIVITY_LOG',
    entry: makeActivityEntry(
      'pod:frontend_ui', 'Frontend UI Pod',
  ...
  dispatch({ type: 'UPDATE_POD', id: 'integration_qa', patch: { status: 'waiting', startTime: Date.now() } });
  await sleep(200);
  dispatch({ type: 'UPDATE_POD', id: 'integration_qa', patch: { status: 'running' } });
  dispatch({
    type: 'ADD_ACTIVITY_LOG',
    entry: makeActivityEntry(
      'pod:integration_qa', 'Integration QA Pod',
      'executing', 'pod_started',
  ...
  dispatch({ type: 'UPDATE_POD', id: 'integration_qa', patch: { status: 'completed', endTime: Date.now(), usedProvider: 'anthropic' } });
  dispatch({
    type: 'ADD_ACTIVITY_LOG',
    entry: makeActivityEntry(
      'pod:integration_qa', 'Integration QA Pod',
  ...
  dispatch({ type: 'SET_VERIFICATION', verification: { ...DEMO_VERIFICATION, verifiedAt: Date.now() } });
  dispatch({
    type: 'ADD_ACTIVITY_LOG',
    entry: makeActivityEntry(
      'nexus-verifier', 'NEXUS Verifier',
  ...
  dispatch({
    ...
    entry: makeActivityEntry(
      'nexus-manager', 'NEXUS Manager',
      'verifying', 'coordination_correction',
