...
export const CONNECTOR_DEFINITIONS: ConnectorDefinition[] = [
  {
    ...
    authTypes: ['api_key', 'bearer', 'oauth2'],
    capabilities: ['inference', 'embeddings', 'reranking'],
    serverSideOnly: true,
    endpointPlaceholder: 'https://ai-gateway.company.com/v1',
  },
  {
    id: 'postgresql',
    name: 'PostgreSQL',
    ...
    authTypes: ['connection_string', 'oauth2'],
    capabilities: ['sql-read', 'sql-write', 'schema-discovery'],
    serverSideOnly: true,
    endpointPlaceholder: 'https://data-broker.company.com/postgres',
  },
  {
    id: 'mongodb',
    name: 'MongoDB',
    ...
    authTypes: ['connection_string', 'oauth2'],
    capabilities: ['document-read', 'document-write', 'schema-discovery'],
    serverSideOnly: true,
    endpointPlaceholder: 'https://data-broker.company.com/mongodb',
  },
  {
    id: 'github',
    name: 'GitHub',
    kind: 'repository',
    description:
      'Inspect repositories, issues, pull requests, checks, and approved write operations.',
    authTypes: ['bearer', 'oauth2'],
    capabilities: ['repo-read', 'repo-write', 'pull-requests', 'issues', 'ci-status'],
    serverSideOnly: true,
  ...
  },
  {
    ...
    capabilities: ['repo-read', 'repo-write', 'pull-requests', 'issues', 'ci-status'],
    serverSideOnly: true,
    endpointPlaceholder: 'https://gitlab.com/api/v4',
    requiredScopes: ['read_api'],
  },
  {
    id: 'mcp-agent',
    name: 'MCP / Agent Endpoint',
    kind: 'agent',
    description: 'Connect a remote tool or agent through a governed MCP-compatible gateway.',
    authTypes: ['bearer', 'oauth2'],
    capabilities: ['agent-delegation', 'tool-discovery', 'tool-execution'],
    serverSideOnly: true,
    endpointPlaceholder: 'https://agent-gateway.company.com/mcp',
  },
  {
    id: 'rest-api',
    name: 'REST API',
    ...
    authTypes: ['none', 'api_key', 'bearer', 'oauth2', 'basic'],
    capabilities: ['api-read', 'api-write'],
    serverSideOnly: true,
    endpointPlaceholder: 'https://api.company.com/v1',
  },
];

export const CONNECTOR_DEFINITION_MAP = new Map(
  CONNECTOR_DEFINITIONS.map((item) => [item.id, item]),
);
...
function hasCredential(config: ConnectorConfig): boolean {
  switch (config.authType) {
    case 'none':
      return true;
    case 'api_key':
      return Boolean(config.credentials.apiKey?.trim());
    case 'bearer':
      return Boolean(config.credentials.token?.trim());
    case 'oauth2':
      return Boolean(config.credentials.token?.trim());
    case 'basic':
      return Boolean(config.credentials.username?.trim() && config.credentials.password?.trim());
    case 'connection_string':
      return Boolean(config.credentials.connectionString?.trim());
  }
}
...
export function diagnoseConnector(
...
): Pick<ConnectorConfig, 'status' | 'issues' | 'diagnostics' | 'lastCheckAt'> {
  ...
  if (!definition) {
    issues.push({
      ...
      severity: 'error',
      title: 'Connector type is unavailable',
      detail: 'The saved connector references a definition that is not installed.',
      remediation: [
        'Choose an installed connector type.',
        'Ask an administrator to install the required connector package.',
      ],
      retriable: false,
    });
  } else {
    ...
    try {
      ...
      issues.push({
        ...
        severity: 'error',
        title: 'Endpoint rejected by security policy',
        detail: (error as Error).message,
        remediation: [
          'Use a public HTTPS broker endpoint.',
          'Remove URL credentials, fragments, and private or loopback hosts.',
        ],
        retriable: true,
      });
    }
    ...
    if (!definition.authTypes.includes(config.authType)) {
      issues.push({
        ...
        severity: 'error',
        title: 'Authentication method is not supported',
        detail: `${definition.name} does not accept ${config.authType}.`,
        remediation: [`Choose one of: ${definition.authTypes.join(', ')}.`],
        retriable: true,
      });
      ...
      issues.push({
        ...
        detail:
          'The selected authentication method requires a memory-only credential for this browser session.',
        remediation: [
          'Enter the credential again.',
          'For production, configure the secret in the server-side vault and issue a short-lived session grant.',
        ],
        retriable: true,
      });
    } else {
    ...
    if (missingScopes.length > 0) {
      issues.push({
        code: 'SCOPE_MISSING',
        severity: 'warning',
        title: 'Required scopes are missing',
        detail: `Missing scopes: ${missingScopes.join(', ')}.`,
        remediation: [
          'Grant only the listed minimum scopes.',
          'Re-run diagnostics after updating authorization.',
        ],
        retriable: true,
      });
    }
    ...
    if (!config.approved) {
      issues.push({
        ...
        remediation: [
          'Review the endpoint, scopes, and control mode.',
          'Approve the connector when the configuration is correct.',
        ],
        retriable: true,
      });
    }
    ...
    if (definition.serverSideOnly) {
      diagnostics.push(
        'Production execution requires the server-side connector broker; direct browser data access is intentionally disabled.',
      );
    }
  }
...
export function routeConnectorCapabilities(
...
): keyof ConnectorConfig['credentials'] | null {
