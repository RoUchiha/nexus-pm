    ...
    <div className="card" style={{ marginBottom: 20 }}>
      ...
      <div style={{ height: 4, background: 'var(--border)', position: 'relative' }}>
        <div style={{
          ...
          width: `${pct}%`, background: complianceColor, transition: 'width 0.6s ease',
        }} />
        {partial > 0 && (
          <div style={{
            position: 'absolute', left: `${(passed / total) * 100}%`, top: 0, height: '100%',
            width: `${(partial / total) * 50}%`, background: 'var(--yellow)', opacity: 0.6,
          }} />
        )}
      </div>
      ...
      <div style={{ padding: '14px 16px' }}>
            ...
            {verification.vcResults.map(result => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 700, color: 'var(--dim)', flexShrink: 0, minWidth: 48 }}>
                      {result.id}
                    </span>
                    ...
                    <span style={{ fontSize: 14, color: meta.color, flexShrink: 0, fontWeight: 700 }}>
                      {meta.icon}
                    </span>
                    ...
                    <div style={{ padding: '10px 12px', borderTop: '1px solid var(--border)', background: 'var(--surface)' }}>
                      {result.evidence && (
                        <div style={{ marginBottom: 8 }}>
                          <div style={{ fontSize: 10, color: 'var(--green)', fontWeight: 700, marginBottom: 4 }}>EVIDENCE</div>
                          <div style={{
                            fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)',
                            background: 'var(--card)', padding: '8px 10px', borderRadius: 4,
                            borderLeft: '3px solid var(--green)', lineHeight: 1.5,
                          }}>
                            {result.evidence}
                          </div>
                        </div>
                      ...
                      {result.gap && (
                        <div>
                          <div style={{ fontSize: 10, color: 'var(--red)', fontWeight: 700, marginBottom: 4 }}>GAP</div>
                          <div style={{
                            fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)',
                            background: 'var(--card)', padding: '8px 10px', borderRadius: 4,
                            borderLeft: '3px solid var(--red)', lineHeight: 1.5,
                          }}>
                            {result.gap}
                          </div>
                        </div>
        ...
        {verification.violations.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 10, color: 'var(--red)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>
              Spec Violations ({verification.violations.length})
            </div>
            {verification.violations.map((v, i) => (
              <div key={i} style={{
                display: 'flex', gap: 10, padding: '7px 10px', borderRadius: 5,
                ...
                <span style={{ color: 'var(--dim)', fontSize: 10, flexShrink: 0, fontFamily: 'var(--font-mono)' }}>{v.type}</span>
                <span style={{ color: 'var(--muted)', flex: 1 }}>{v.description}</span>
                <span style={{ color: 'var(--dim)', fontSize: 10, fontFamily: 'var(--font-mono)', flexShrink: 0 }}>{v.podId}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {verification.specUpdates.length > 0 && (
          <div>
            <div style={{ fontSize: 10, color: 'var(--blue)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>
            ...
            {verification.specUpdates.map((u, i) => (
              <div key={i} style={{
                ...
                <span style={{ fontSize: 10, color: 'var(--blue)', fontWeight: 700, marginRight: 8 }}>{u.section}</span>
                <span style={{ color: 'var(--muted)' }}>{u.suggestion}</span>
                {u.rationale && (
                  <div style={{ fontSize: 11, color: 'var(--dim)', marginTop: 3 }}>↳ {u.rationale}</div>
                )}
              </div>
            ))}
          </div>
