...
export function validateManagerPlan(raw: unknown): void {
  ...
  array(spec.outcomes, 'Spec outcomes', 1, 20).forEach((item, index) =>
    safeString(item, `Outcome ${index + 1}`),
  );

  const criteria = array(spec.verificationCriteria, 'Verification criteria', 1, 50).map(
    (item, index) => {
  ...
  if (new Set(criteria).size !== criteria.length)
    throw new Error('Verification criterion ids must be unique.');
  const criterionIds = new Set(criteria);

  const pods = array(root.pods, 'Execution pods', 1, 8).map((item, index) => {
    ...
    const dependencies = array(pod.dependencies, `${id} dependencies`, 0, 8).map((dep, depIndex) =>
      safeString(dep, `${id} dependency ${depIndex + 1}`, 64),
    );
    const vcIds = array(pod.vcIds, `${id} verification criteria`, 0, 50).map((vc, vcIndex) =>
      safeString(vc, `${id} VC ${vcIndex + 1}`, 16),
    );
  ...
  for (const pod of pods) {
    if (new Set(pod.dependencies).size !== pod.dependencies.length)
      throw new Error(`${pod.id} contains duplicate dependencies.`);
    for (const dependency of pod.dependencies) {
      if (!podIds.has(dependency))
        throw new Error(`${pod.id} references unknown dependency ${dependency}.`);
      if (dependency === pod.id) throw new Error(`${pod.id} cannot depend on itself.`);
    }
    for (const vcId of pod.vcIds)
      if (!criterionIds.has(vcId))
        throw new Error(`${pod.id} references unknown criterion ${vcId}.`);
  }

  const assigned = new Set(pods.flatMap((pod) => pod.vcIds));
  const unassigned = criteria.filter((id) => !assigned.has(id));
  if (unassigned.length > 0)
    throw new Error(`Verification criteria must be assigned to a pod: ${unassigned.join(', ')}.`);
